...
public interface SearchRepository extends CrudRepository<RegisterDtoEntity, String> {
	 ...
	 @Query("SELECT t FROM RegisterDtoEntity t WHERE t.referencenumber= IFNULL(:referencenumber,referencenumber) AND t.usertype= IFNULL(:usertype,usertype) AND t.firstname= IFNULL(:firstname,firstname) AND t.lastname = IFNULL(:lastname,lastname) AND t.countrybirth = IFNULL(:countrybirth,0) AND t.surveystatus = IFNULL(:status,surveystatus)")
